...
export const getDadhboardStats = TryCatch(async(req,res,next)=>{
    ...
    if(myCache.has(key)) stats = JSON.parse(myCache.get(key) as string);
        ...
        const count = {
                revenue, product:productsCount,user:usersCount,order:allOrders.length
        }
...
export const getPieCharts = TryCatch(async(req,res,next)=>{
    ...
    if(myCache.has(key)) charts = JSON.parse(String(myCache.get(key)))
...
export const getLineCharts = TryCatch(async(req,res,next)=>{
    ...
    if(myCache.has(key)) charts = JSON.parse(myCache.get(key) as string);
        ...
        const revanue = getChartData({length:12, today, docArr:orders, property:"total"});            
